# -*- coding: utf-8 -*-
"""
collect_saliency.py -- read the terrain-corruption diagnostic and apply its decision rule.

Pure stdlib (no torch, no numpy) so it runs on the dev machine. Point it at the folder
run_saliency.sh wrote:
...
floor. The floor matters: this sim is not bit-reproducible run to run, so a check written as
an exact match could only ever fail.
"""
...
def main():
    ...
    for pol in POLICIES:
        ...
        if clean["channel_l1"] < 1e-3:
            ...
            print("      nothing to ignore. Re-run at a fixed terrain level instead.")
        ...
        for a in ARMS:
            ...
            if floor is None:
                continue
            # 1.5x the measured floor: it comes from a single pair of clean runs, so it is
            # a noisy estimate and a bare comparison over-reports. The absolute term keeps a
            # metric whose two clean runs happened to agree exactly from making every
            # rounding difference "significant".
            ...
            if beats:
                ...
                print("      -> the policy is invariant to its terrain input.")
        print("")
